...
export function loadProfile(): CalibrationProfile {
  ...
  try {
    const parsed = JSON.parse(localStorage.getItem(STORAGE_KEY) ?? 'null') as CalibrationProfile | null;
    if (
      !parsed ||
      parsed.version !== 1 ||
...
export function saveProfile(profile: CalibrationProfile): void {
  if (typeof window !== 'undefined') localStorage.setItem(STORAGE_KEY, JSON.stringify(profile));
}
...
function motionDistance(a: number[][], b: number[][]): number {
  ...
  for (let t = 1; t < FRAMES; t += 1) {
    ...
    total += Math.sqrt(frame / FEATURES);
  }
...
function sequenceDistance(a: number[][], b: number[][]): number {
  const shape =
    a.reduce((sum, frame, index) => sum + frameDistance(frame, b[index]), 0) / FRAMES;
  return shape * 0.72 + motionDistance(a, b) * 0.28;
}
...
export function matchPersonal(
  ...
  const best = ranked[0];
  const second = ranked.find((candidate) => candidate.label !== best.label) ?? best;
  const classTop = ranked
    .filter((candidate) => candidate.label === best.label)
    .slice(0, 3)
    .map((candidate) => candidate.distance);

  const classDistance = classTop.reduce((sum, value) => sum + value, 0) / classTop.length;
  const score = toSimilarity(classDistance);
  const margin = score - toSimilarity(second.distance);
